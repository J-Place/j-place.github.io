...
**Reference captures.** Every run writes a full-page, unmasked screenshot of each page (pass or fail) to `visual-captures/<slug>--<project>.png` (gitignored, overwritten each run) and attaches it to the HTML report, so passing pages can be eyeballed too — not just failures' `-actual`/`-diff` pairs. The attachment name ends in `-actual` so the report renders a test-row thumbnail for passing pages the same way it does for failures (the report only thumbnails image attachments whose name matches `/-(expected|actual|diff)/`). This is separate from the baseline comparison; keep it in the spec.
