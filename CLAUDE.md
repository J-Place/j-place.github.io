...
**Club Finder and Calendar of Events location are pinned to Sarasota, FL.** `clubs-filter.js` and `events-filter.js` both open at "Sarasota, FL" (USMS HQ) then override it with the runner's IP-based city via an `ipinfo.io` fetch — non-deterministic per machine/network. The spec aborts every `ipinfo.io` request so both pages are always captured at the Sarasota default (Club Finder: 2 Sarasota clubs, 25 mi; Events: 4 events within the default 250 mi range, soonest is "2026 Shark Tank SCM Meet"). Keep that route block; only `clubs-filter.js` and `events-filter.js` call `ipinfo.io`.
