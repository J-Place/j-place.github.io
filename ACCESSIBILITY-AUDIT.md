...
| 18 | Touch target size, 2 instances | Hero carousel nav dots are 10×10px with minimal spacing (WCAG 2.5.8 wants ≥24×24px). | **Fixed** — see Changelog #18. |
...
**Date:** 2026-08-31 (corrected 2026-08-31)
**Files changed:** `src/_includes/partials/Navigation/Footer.njk`, `src/_includes/layouts/lmsc.njk`
**What changed:** Original fix restored hardcoded copy from `production/src/App/views/Social/Subscribe.jsx:10-11` ("Subscribe To See More" / "Send us you email to see more articles and track your Fitness Logs.") into the empty `<h4>`/`<p>`, reasoning that the mockup had dropped real production text. **That was wrong** — the JSX file is a standalone component's default render, not what's actually live. Fetching the live production homepage directly (`curl https://www.usms.org/`) shows the real, currently-rendered footer ships this exact `<h4 class="subscribe__header m-b-30"></h4>` and `<p class="m-b-30"></p>` genuinely empty (almost certainly an unpopulated CMS field upstream) — there is no real copy to restore. Reverted the fabricated text and instead removed the two empty elements entirely, leaving only the "Join Now" button. An empty heading/paragraph carries no accessible value regardless of why it's empty, so removing them (rather than inventing content) is the correct fix and doesn't misrepresent what production actually shows.
**Why:** A heading with no text prevents screen reader users from getting any information about the section's structure at that point — same underlying issue as the original finding, just fixed without fabricating copy that isn't real.
**Verified by:** axe-core (`empty-heading`) against both `http://localhost:8080/home/` (uses `Footer.njk`) and `http://localhost:8080/lmsc/ohio-masters-swimming/` (uses `lmsc.njk`): 0 violations on both.
...
- **Footer "JOIN NOW" button** (`.subscribe a.btn-primary` inside `Footer.njk`/`lmsc.njk`): reported at `#dae2e9` on `#0e69a8` (4.44:1) by both WAVE and axe-core, even though the button's own declared colors are white on `#0068ad` (5.86:1 — passing on paper). Root cause: `.footer-right { opacity: 0.8 }` (production `Footer.css:48`) composites every descendant color toward the dark backdrop behind it — both the background *and* the text — so the *rendered* pixels are darker/muddier than the *declared* colors; confirmed by comparing `getComputedStyle` (passing) against axe-core's pixel-sampled contrast (failing), and confirmed the identical failure exists on the live production site today. Scoped narrowly to `.subscribe a.btn-primary` so the unrelated, already-passing hero "JOIN NOW" button (site-wide `.btn-primary` default) is untouched.
  **2026-09-23 update:** switched from a dark button (white text on `#003a5c` background, ~7.86:1) to a white-background button with `#003a5c` text — same composited pair inverted, same ~7.86:1. Note for anyone revisiting this: because both button colors sit inside the same opacity blend, swapping *which* color is text vs. background doesn't dodge the contrast requirement — a white-background attempt using the original `#0068ad` as text composites to the exact same failing `#dae2e9`-on-`#0e69a8` pair as the original bug (~4.44:1). Any future color change here has to be re-run through the 0.8 blend, not just checked against a white/dark backdrop directly.
...
### [18] — Homepage hero carousel nav dots: touch target size
**Date:** 2026-09-23
**Files changed:** `src/css/Media/Carousel.css` (new), `src/_includes/layouts/home.njk`
